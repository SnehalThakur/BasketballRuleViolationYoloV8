...
class DoubleDribbleDetector:
    ...
    def process_frame(self, frame):
        ...
        # rounded_results = np.round(pose_results[0].keypoints.cpu().numpy(), 2)
        ...
        for ball_results in ball_results_list:
            for bbox in ball_results.boxes.xyxy:
                ...
                left_distance = np.hypot(
                    ball_x_center.cpu() - left_wrist[0], ball_y_center.cpu() - left_wrist[1]
                )
                right_distance = np.hypot(
                    ball_x_center.cpu() - right_wrist[0], ball_y_center.cpu() - right_wrist[1]
                )
